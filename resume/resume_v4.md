...
외부 플랫폼 댓글 등록이 건당 3~5분씩 걸리는 작업인데, 일괄 댓글이나 마케팅 캠페인처럼 수천에서 수만 건이 동시에 트리거되는 시나리오에서는 cmong-be에서 scraper로 동기 HTTP 호출이 이어지면서 이벤트 루프가 막히고 HTTP 타임아웃이 빈번하게 발생했습니다. 사용자 시간대를 맞춰 등록할 수도, 한 번 출발한 작업을 취소할 수도 없는 구조였습니다.
...
일괄 댓글(priority 9, 피크 1,000건)과 마케팅 캠페인(priority 5, 수만 건)을 같은 코드 경로로 흡수하면서, 사용자에게는 즉시 응답을, 운영자에게는 사용자 시간대 맞춤 등록과 Stage 1까지의 취소 윈도우를 확보했습니다. 각 컴포넌트가 죽었을 때의 시나리오는 README에 여섯 가지로 정리해 두었고, Redis 다운 시 fail-open으로 진행하는 정책으로 가용성을 유지합니다.
