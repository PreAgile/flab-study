...
> SVG 이미지 임베드 버전은 `resume_v12.md` · docx 빌드는 `resume_v12.docx` 를 참고하세요.
...
**큐 · 메시징 · 재시도**
- 메시지 큐 DLX 없이 application-level retry — `setTimeout + republishMessage` 로 hot loop 회피, 30s / 5m / 30m 단계별 backoff. Phase 2 에서 DLX 표준 패턴 마이그레이션 예정 명시.
