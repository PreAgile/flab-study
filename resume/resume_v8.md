...
분산 환경에서 트랜잭션 일관성을 지키는 일, 비동기 메시징으로 처리량을 늘리는 일, 그리고 트래픽이 늘면서 동기 호출과 단일 인스턴스만으로 풀리지 않는 문제를 큐, 락, 세션 분리로 다시 짜는 일에 관심이 많습니다.

현재 르몽에서 6개 배달 플랫폼을 통합 관리하는 리뷰 SaaS의 백엔드를 담당하며, Node.js/NestJS 기반 서비스를 운영하면서 분산 작업 큐, 세션 일관성, 멱등성, 외부 API 의존성 격리, 장애 알림과 대응 자동화 같은 문제를 다뤄왔습니다. 일평균 API 호출 20만 건, 페이지 수집 100만 건, 리뷰 수집 12만 건 규모를 RabbitMQ, Redis, Aurora, ECS, Datadog 위에서 운영하고 있습니다.

이 운영 경험을 Kotlin/Spring Boot/JPA 환경에서 동일하게 재현하고 비교하는 재설계 프로젝트를 진행하면서, **JVM 오픈 소스 프로젝트**에도 정기적으로 기여하고 있습니다. **Kotlin 테스트 프레임워크 Kotest의 organization 정식 멤버**로 6개 PR을 머지했고, **LINE의 비동기 RPC 프레임워크 Armeria**에는 MDC 확장 PR을 머지했습니다. Public API 호환성, 비동기 컨텍스트 전파, JSON Schema 표준 구현, 정적 분석(NullAway) 정책 같은 제약을 맞추며 메인테이너 리뷰를 통과한 변경들입니다.

## 오픈소스 기여

JVM 생태계의 주요 프로젝트에 정기적으로 기여하고 있습니다. 새 Public API 설계, 비동기 컨텍스트 전파, JSON 표준 구현, 정적 분석 정책 같은 호환성 제약을 맞춰야 하는 변경을 메인테이너 리뷰를 거쳐 머지했습니다.
...
요청/응답 컨텍스트를 Logback MDC로 export하는 **BuiltInProperty 확장 PR을 머지**(#6683, +339/-5)했습니다. HTTP/2 비동기 타이밍을 ThreadLocal로 노출하면서 RequestContext의 lifecycle을 깨지 않도록 하는 작업이었고, 프레임워크에 적용된 NullAway 정책에 맞춰 nullable 시그니처를 정리했습니다.
...
5년 동안 Node.js로 운영하며 부딪힌 분산 락, 결제 멱등성, 트랜잭션 분리, 외부 의존성 격리 같은 문제를 Java/Kotlin/Spring 환경에서 동일한 시나리오로 재현하고, 같은 문제를 다른 도구로 풀 때의 trade-off를 측정해 ADR로 정리합니다. 현재까지 **ADR 49건, 실험 47건, 기술 블로그 17편**을 누적했습니다.

- **commerce-comment-platform-be** (Java / Spring Boot / JPA): 결제 멱등성 4단계, 분산 락 4종 비교(비관락 180ms / 낙관락 549ms / GET_LOCK 5,015ms / Redisson watchdog), MySQL InnoDB RR과 ANSI RR의 차이 재현, 트랜잭션 분리 패턴(단순분리/Saga/Outbox) 9 시나리오 매트릭스, No-offset Pagination.
