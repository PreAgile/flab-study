...
> **이 파일은 mermaid 다이어그램 버전입니다.**
> SVG 이미지 임베드 버전은 `resume_v13.md` · docx 빌드는 `resume_v13.docx` 를 참고하세요.
> GitHub · GitLab · Notion · Obsidian 에서 다이어그램이 native 렌더링됩니다.
...
**Kotest** — Kotlin 테스트 프레임워크 (4.7k stars)
2026.03 메인테이너 sksamuel 의 직접 초대로 **organization 정식 멤버**, 한 달간 6 PR 머지.

- [#5905](https://github.com/kotest/kotest/pull/5905) — 타입 안전 Test Metadata Public API 신규 설계 (+355/-16, 5 리뷰 16h 내 머지)
- [#5835](https://github.com/kotest/kotest/pull/5835) — 컬렉션 data class 필드 단위 diff
- [#5807](https://github.com/kotest/kotest/pull/5807) — JSON Schema anyOf/oneOf 표준 구현
- [#5795](https://github.com/kotest/kotest/pull/5795) — JSON Matchers 커스텀 Json 인스턴스 지원
- [#5789](https://github.com/kotest/kotest/pull/5789) — `@OnlyInputTypes` 활용 타입 안전 어설션
- [#5756](https://github.com/kotest/kotest/pull/5756) — `shouldHaveSingleElement` 어설션 체이닝

**Armeria** — LINE 의 Java 기반 비동기 RPC 프레임워크 (5.1k stars)

- [#6683](https://github.com/line/armeria/pull/6683) — `RequestContextExporter` BuiltInProperty 확장 + Logback MDC export 테스트 보강 (+339/-5, 메인테이너 1/2 승인, 인라인 리뷰 반영 중)

**Spring Batch** — 이슈 분석 완료, PR 작성 중 (2.9k stars)

- [#3946](https://github.com/spring-projects/spring-batch/issues/3946) — fault-tolerant step chunk scanning 공식 문서 4년 누락 보완 (메인테이너 "documentation issue" 라벨 부여)
- [#4478](https://github.com/spring-projects/spring-batch/issues/4478) — conditional flow decider order 버그 (메인테이너 "this is a bug" 인정)

**Spring Cloud Gateway** — 이슈 분석 완료, 릴리즈 사이클 대기

- [#3311](https://github.com/spring-cloud/spring-cloud-gateway/issues/3311) — Apache HttpClient5 쿠키 매니저 기본 비활성화 (메인테이너 코멘트 완료, 구현 준비, 5.1.x 릴리즈 대기)
- [#4024](https://github.com/spring-cloud/spring-cloud-gateway/issues/4024) — API Versioning Predicates 공식 문서 보완 (머지 가능성 메인테이너 확인)
...
<details>
<summary><b>📐 A1. 전체 시스템 아키텍처</b> (클릭하여 펼치기)</summary>
...
**문제** — 결제 PG webhook 은 같은 결제 식별자로 **중복 호출되는 경로가 5종 이상** 존재합니다. ① PG 자체 retry(5xx/timeout) ② 정기결제 schedule 1차 실패 후 fallback retry 로 PAID/FAILED 가 거의 동시 도착 ③ 결제 취소 후 재결제 ④ 부분 환불 상태 전이 ⑤ Load Balancer keep-alive 재시도. 단순 INSERT 는 결제 row 이중 집계 → 매출 중복 → 알림톡 2회 발송 → 정기결제 중복 생성 → 다음 달 이중 과금까지 도미노로 이어집니다. 멀티 인스턴스로 배포되므로 in-memory dedupe 가 불가능하고, ORM 의 `findOne → save` 패턴은 read-modify-write 사이에 race window 가 존재합니다. 분산 캐시 락 단독으로는 TTL 만료 후 다른 워커의 락을 잘못 지우는 **ABA 문제** (Martin Kleppmann 의 Redlock 분석에서 지적된 hazard) 가 발생할 수 있고, DB UNIQUE 단독으로는 PG retry 폭주를 1차에서 차단할 수 없으며, 상태머신만으로는 동시 INSERT race 를 막을 수 없습니다.

**해결** — 단일 도구로는 모든 race 를 막을 수 없다는 판단으로 **4중 방어선** 을 의도적으로 곱셈으로 결합했습니다. ① 분산 캐시 SETNX EX + 토큰 기반 lock value = ABA-safe distributed lock (Stripe 의 Idempotency Key 디자인과 같은 결, lock value 로 ownership 검증) ② DB UNIQUE on `imp_uid + merchant_uid + status` 복합 키 = 최종 정합성 보루(1시간 dedupe window) ③ 상태머신 READY → PAID/FAILED/CANCELLED × action_type 분기 ④ Lua eval 로 GET → 비교 → DEL 원자 실행, 자신의 락만 해제. 각 방어선은 한 단계 약점을 다음 단계가 보완하도록 설계되었습니다 — 분산 캐시가 일시 장애로 fail-open 되어도 DB UNIQUE 가 최종 차단, 상태 역전(PAID 뒤 FAILED 도착) 시 복합 키에 status 가 포함되어 다른 row 로 안전 격리. 동일한 SETNX + Lua DEL 패턴은 결제 도메인 외 8 곳 이상에서 재사용되어 **결제 외 8 곳 이상 흐름에서 공통 helper 로 재사용**했습니다.
...
**해결** — 단일 도구로는 모든 race 를 막을 수 없다는 판단으로 **4중 방어선** 을 의도적으로 곱셈으로 결합했습니다. ① 분산 캐시 SETNX EX + 토큰 기반 lock value = ABA-safe distributed lock (Stripe 의 Idempotency Key 디자인과 같은 결, lock value 로 ownership 검증) ② DB UNIQUE on `imp_uid + merchant_uid + status` 복합 키 = 최종 정합성 보루(1시간 dedupe window) ③ 상태머신 READY → PAID/FAILED/CANCELLED × action_type 분기 ④ Lua eval 로 GET → 비교 → DEL 원자 실행, 자신의 락만 해제. 각 방어선은 한 단계 약점을 다음 단계가 보완하도록 설계되었습니다 — 분산 캐시가 일시 장애로 fail-open 되어도 DB UNIQUE 가 최종 차단, 상태 역전(PAID 뒤 FAILED 도착) 시 복합 키에 status 가 포함되어 다른 row 로 안전 격리. 동일한 SETNX + Lua DEL 패턴은 결제 도메인 외 8 곳 이상에서 재사용되어 **사내 분산 락 표준 라이브러리** 로 정착했습니다.

<details>
<summary><b>📐 A2. webhook 4중 방어 시퀀스</b> (클릭하여 펼치기)</summary>
...
**JVM/Spring 매핑** — Spring `@Transactional`(propagation/isolation 설계) · Redisson distributed lock + watchdog · DB unique constraint + JPA `OptimisticLockException` 핸들링 · `@RestControllerAdvice` 의 ER_DUP_ENTRY 매핑.

---

### 사례 2. 결제 정합성 reconciliation — 좀비 schedule · 중복 빌링키 · admin override orphan 자동 탐지 ｜ 2025.10 ~ 운영 중

**문제** — 사례 1 의 4중 멱등성으로도 못 막는 **eventual inconsistency** 3종이 존재합니다. ① **좀비 schedule** — 외부 PG 서버에는 schedule 이 살아있는데 DB 에는 cancel 로 기록되어 다음 결제일 자동 과금 ② **중복 빌링키** — 한 사용자에 SCHEDULED billing 이 2개 이상 잡혀 다음 결제일 이중 과금 ③ **admin override orphan** — 어드민이 구독을 1년 뒤까지 수동 연장했는데 이전에 잡혔던 자동결제 schedule 이 그대로 남아 active 기간 중 또 결제. 셋 다 사용자 클레임과 환불 분쟁이 발생한 뒤에야 노출되는 「조용한 결함」 입니다. 외부 PG API 호출은 분당 한도와 비용 제약이 있어 전체 N 명에 매일 호출은 불가능하고, DB 의 SCHEDULED 와 PG 의 schedule_status 는 본질적으로 의미가 다른(DB 는 billing 단위, PG 는 schedule 단위) eventual consistency 문제입니다.

**해결** — 결제 운영의 정석인 **「의심 대상 K << N 으로 압축 → 외부 호출 최소화 → 4-way 분기 자동 정정」** 패턴을 일일 cron + 어드민 수동 트리거 이중 진입점으로 구현. DB 차집합으로 의심 customer 만 추리고, 추려진 K 명에 대해서만 외부 PG schedule API 호출, 응답을 4-way 분기 처리 — schedule 0개면 DB 좀비로 정정, 1개면 정상, 2개 이상이면 다른 schedule 자동 cancel, API 무응답이면 Slack 알림. admin override orphan 은 「활성 구독 종료일이 다음 자동결제 예약보다 1개월 이상 뒤」 라는 정확한 조건의 단일 SQL 로 탐지합니다. 일일 cron 은 SafeCron 데코레이터(사례 14)로 멀티 인스턴스에서 한 번만 실행되도록 분산 락 적용.
...
**JVM/Spring 매핑** — Spring Scheduler + ShedLock (멀티 인스턴스 cron 직렬화) · JdbcTemplate / QueryDSL 로 차집합 쿼리 · Spring Retry + RateLimiter 로 외부 PG 호출 보호 · ApplicationEventPublisher 로 Slack 알림 비동기.
...
**해결** — **3중 idempotency 레이어**: ① **사전 검증** — `createUserCoupon` 진입 시 (user, coupon) 으로 기존 row 확인, 있으면 기존 row 를 그대로 반환(idempotent return) ② **트랜잭션** — 쿠폰 수정은 ORM QueryRunner 기반 명시적 트랜잭션(`startTransaction / commit / rollback / release`)으로 delete + add + update 를 한 단위로 묶음 ③ **schedule fallback 보정** — schedule 등록 직전에 사용 가능한 user-coupons 를 재조회 후 「다음 결제 시점에 실제 적용될 금액」 을 다시 계산 → 불일치 시 schedule amount + coupon_id 보정. 한 함수가 결제·schedule·change_plan·promotional upgrade 의 모든 흐름에서 재사용되어 DRY 유지.
...
**JVM/Spring 매핑** — JPA `@OneToMany` + `@Version` 낙관락 · Spring `TransactionTemplate` 명시적 트랜잭션 · Hibernate `@PostLoad` 로 schedule fallback 시점 재계산 · Bean Validation `@AssertTrue` 로 amount 정합성 가드.
...
**JVM/Spring 매핑** — JPA `LockModeType.PESSIMISTIC_WRITE` · Spring `@Transactional(propagation=REQUIRES_NEW)` 로 그룹 단위 트랜잭션 경계 · JpaSpecificationExecutor 로 동적 조건 · `findAllById` + `saveAll` batch.
...
- 예약 결제 금액 불일치 시 Slack 자동 경고 — 결제 정합성 모니터링 자동화.
- APM span tagging 으로 결제 정합성 메트릭(amount_mismatch, coupon_select)이 자동 집계.
- 사내 분산 락 helper(`delIfValueMatches`) 8 곳 이상 재사용 — promotional upgrade · change plan · verify code · 알림 배치 등으로 표준화.
...
**해결** — **`SELECT ... FOR UPDATE` 비관락 + 그룹 단위 트랜잭션 경계** 의 표준 패턴. 한 사장님(`groupKey = email + business_number`) 묶음을 별도 `@Transactional()` 메서드로 분리하여 **그룹 = 트랜잭션 경계**. 트랜잭션 내에서 `(platform, platform_id)` 행에 X-lock 을 잡고, 충돌 시 그룹 전체를 명확한 사용자 메시지로 marking. 재시도 멱등성은 단계별 `findOne` 우선 + 함수형 update(`attempt_count + 1`)로 race-free 카운터. 상태머신은 `PENDING → VALIDATED → REGISTERING → REGISTERED | REGISTER_FAILED` 로 가시화하고 `last_error · last_attempted_at · attempt_count` 를 row 에 박아 운영자가 재업로드 없이 실패만 재시도할 수 있게 했습니다.
...
<details>
<summary><b>📐 B1. 관계형 DB 작업 큐 + CAS</b> (클릭하여 펼치기)</summary>
...
**JVM/Spring 매핑** — Spring Batch 의 `JobRepository` 대안으로 직접 운영 가능 · JPA `@Version` 낙관락 + `OptimisticLockException` 으로 CAS · Coroutines + supervisorScope (Kotlin) 또는 `@Async ThreadPoolTaskExecutor` (Java) · Micrometer 로 진행률 메트릭.
...
<details>
<summary><b>📐 B2. Hierarchical 동시성 (계정 단위 순차 · 계정 사이 병렬)</b> (클릭하여 펼치기)</summary>

```mermaid
flowchart LR
    Input["입력<br/>13,127 매장<br/>5,719 platform_id 그룹<br/>13,699 task"]
    Group["그룹 격리<br/>key = (platform_id, password)<br/>value = shop_id 리스트"]
    Pool["ThreadPool<br/>max_workers = W"]

    Input --> Group --> Pool

    Pool --> T1["⚙️ Thread 1<br/>→ 그룹 A"]
    Pool --> T2["⚙️ Thread 2<br/>→ 그룹 B"]
    Pool --> T3["⚙️ Thread 3<br/>→ 그룹 C"]
    Pool --> TN["⚙️ Thread W<br/>→ 그룹 Z"]

    T1 --> SA1["🏪 매장 1"] --> SA2["🏪 매장 2"] --> SA3["🏪 매장 3"]
    T2 --> SB1["🏪 매장 X"] --> SB2["🏪 매장 Y"]
```
...
| 플랫폼 특성 | 임계치 |
|---|---|
| 봇 탐지 심한 플랫폼 (검색 포털) | **0** (첫 인증 오류 = 영구 장애 가정) |
| 배달앱 A | 3 |
| 배달앱 B | 2 |
| 배달앱 C | 3 |

**운영 검증 결과 (단일 플랫폼 1 회 실행)**

| 지표 | 값 |
|---|---|
| 2 시간 cron 안에 처리 | **13,699 task / 5,719 그룹 / 13,127 매장** |
| 동일 platform_id 동시 호출 | **0건** |
| 장애 격리 | 인증 에러 그룹만 비활성 · 다른 그룹 정상 진행 |
...
**JVM/Spring 매핑** — Kotlin Coroutines `Semaphore` + `supervisorScope` 로 hierarchical 동시성 · 또는 Java `ExecutorService` + `CompletableFuture` · Resilience4j `Bulkhead` 로 계정 단위 동시성 격리 · `CircuitBreaker` 로 도미노 차단.
...
**JVM/Spring 매핑** — Spring Boot Actuator `/health` + readinessProbe / livenessProbe · Spring Cloud Gateway 또는 Reverse Proxy 의 dynamic config · K8s rolling update 대신 동등 패턴 적용 가능.
...
<details>
<summary><b>📐 B3. BIZ 대시보드 2 단계 쿼리 + SQL VIEW SSOT</b> (클릭하여 펼치기)</summary>
...
sequenceDiagram
    ...
    rect rgb(248, 250, 252)
        Note over API,DB: Step 1 — Shop 목록 추출 (4단 JOIN)
        API->>DB: brands ⋈ brand_users ⋈ shops ⋈ brand_shops<br/>ORDER BY ... LIMIT 21 (has_more 트릭)
        DB-->>API: shop_ids = [id1, id2, ..., id20]
    end

    rect rgb(236, 253, 245)
        Note over API,DB: Step 2 — Orders GROUP BY (추린 ids 만)
        API->>DB: WHERE shop_id IN (...20 ids)<br/>GROUP BY shop_id, sales_date
        DB-->>API: 140 행 (20 × 7일)
    end

    Note over API: 매장 4상태 분류<br/>active / inactive / shop_deleted / brand_removed
    API-->>U: 대시보드 응답 (외부 IO 0건)
```
...
| 안티패턴 | Sweet spot |
|---|---|
| 단일 거대 JOIN | 2 단계 batch select |
| 1,000 × 7 × N 폭발 | 20 ids 추린 후 IN lookup |
| ORM 동적 조건 한계 | raw SQL + parameterized query |
| 외부 IO 응답 경로 진입 | daily upsert + 4상태 머신 |

**SQL VIEW SSOT** — `v_shop_crawling_status` 가 메인 API · 어드민 ad-hoc · BI 도구의 stale 정의 단일 출처.
...
**JVM/Spring 매핑** — Spring Data JPA + QueryDSL 로 동적 조건 / dynamic order by · Native Query 로 2단계 batch select · Hibernate `@FormulaJoinTable` 또는 SQL VIEW 매핑 (`@Subselect`) · `@EntityGraph` 로 N+1 회피.
...
- 운영 로그 「Job N modified by another instance (optimistic lock failed)」 정상 검출 — race 발생하나 차단됨이 데이터로 입증.
- **외부 플랫폼 중복 댓글 0건**, 인스턴스 재배포 무중단.
- 외부 큐 솔루션 없이 **관계형 DB 를 큐로 쓰는 검증된 패턴으로 재사용** — "MySQL is the queue".
...
<details>
<summary><b>📐 C1. 세션 락 레지스트리 + Cold-Start Guard</b> (클릭하여 펼치기)</summary>
...
**JVM/Spring 매핑** — Redisson `RFairLock` (FIFO + lease auto-extend) 또는 ShedLock + RLock 합성 · Kotlin Coroutines `Mutex` + `withLock` 으로 in-process layer · `@PreDestroy` 로 graceful release · Micrometer gauge 로 큐 깊이 노출.

---

### 사례 10. 자체 IP 평판 시스템 — 외부 의존 제거 + 비용 88.75% 절감 (월 800만 → 90만) ｜ 2025.07 ~ 운영 중

**문제** — 외부 Residential 프록시 의존도가 높아 비용이 매월 **800만 원**, 외부 벤더 장애가 그대로 서비스 중단으로 이어지는 **SPOF** 구조였습니다. Datacenter 프록시(월 90만)로 전환하면 비용은 절감되지만 datacenter IP 는 시간에 따라 port 별로 IP 가 바뀌어(Identifier 가 IP 에서 port 로 전환됨) **평판 측정은 IP 기준이어야 의미 있는데 IP 는 stable 하지 않은** 본질적 불일치가 발생합니다. 추가로 Cluster CROSSSLOT, pool exhausted 시 legacy fallback 의 함정(차단된 IP 재할당), 가장 미묘한 hazard 인 **port rotation 회피**(차단된 IP 가 다음에 다른 port 에 mapping 되면 그 port 가 영구 blocklist 를 회피)까지 6개월에 걸쳐 풀어야 했습니다.
...
<details>
<summary><b>📐 C2. 자체 IP 평판 시스템 Before / After</b> (클릭하여 펼치기)</summary>
...
**JVM/Spring 매핑** — Spring Cloud Gateway + Resilience4j `Retry` / `CircuitBreaker` / `Bulkhead` 합성 · `WebClient` ExchangeFilterFunction 으로 proxy resolver 추상화 · Spring Configuration `@ConfigurationProperties` + `@Validated` 로 env 정책 명문화.

---

### 사례 11. Akamai Bot Manager 우회 — 인증 쿠키 상태 머신 · Referrer Warming · Click Loop (77.8% → 100%) ｜ 2026.02

**문제** — Akamai Bot Manager 가 깔린 외부 플랫폼은 평범한 헤드리스 브라우저로는 99% 차단됩니다. stealth fork 로 fingerprint 를 우회해도 sensor 가 준비되기 전 로그인 제출 시 인증 쿠키가 pending 상태로 남아 sensor 검증 실패 → 403 차단. 옛 구현의 4 가지 silent 결함: ① 검증 토큰과 challenge 토큰이 혼재된 상태에서 「검증 토큰 첫 등장 즉시 break」 하여 race window 안에 뒤늦게 도착하는 challenge 토큰을 놓침 ② 검증 토큰과 차단 토큰의 혼재 케이스를 「검증 성공」 으로 오분류 ③ Akamai 차단 응답을 application-level 401/403 으로 오분류해 password retry 로 흘려보냄(Full Retry 미작동) ④ sensor 제출 후 즉시 break 해 sensor 의 추가 검증(1~2초) 윈도우를 놓침.
...
<details>
<summary><b>📐 C3. Akamai 우회 + 인증 쿠키 4-state 머신</b> (클릭하여 펼치기)</summary>
...
**JVM/Spring 매핑** — Spring `@Component` 인증 쿠키 상태 분류기 (sealed interface) · `WebClient` interceptor 로 응답 헤더 기반 차단 검출 · Resilience4j `TimeLimiter` + `Retry` 합성 · Kotlin sealed class `PollingAction` 으로 discriminated union.

---

### 사례 12. Single-Flight Coordinator 포트화 — Hexagonal + Decorator 5종 합성, 7가지 행동 계약 spec 명문화 ｜ 2026.01

**문제** — 한 사용자(`platformId`)에 대한 동시 요청 N개를 받으면 첫 번째만 실제 로그인하고 나머지는 결과 공유해야 합니다(single-flight, coalescing). 첫 구현은 한 서비스 안에 인-메모리 Map 80+ 줄 인라인이었고, 다른 플랫폼 서비스도 같은 패턴이 필요한데 재사용되지 않았으며, 분산 캐시 어댑터 추가 시 해당 서비스를 또 손대야 했습니다. 더 본질적인 문제는 **single-flight 가 외부에 약속하는 행동 계약 자체가 명문화되어 있지 않아 회귀를 잡을 수 없다는 점** — ① 결과 일관성(같은 cause 인스턴스 공유, 스택 트레이스 보존) ② 세대 가드(forceRelease 후 새 record 를 옛 record 의 늦은 finally cleanup 이 지우면 안 됨) ③ 동기 throw 정규화(operation 이 sync throw 해도 Map record 박힌 뒤 throw) 같은 미묘한 invariant 들.
...
<details>
<summary><b>📐 C4. Single-Flight Coordinator — Hexagonal + Decorator 5종</b> (클릭하여 펼치기)</summary>

```mermaid
flowchart LR
    subgraph Callers["호출 서비스 (Port 만 의존)"]
        S1["검색 포털 인증<br/>(80+ 줄 인라인 제거)"]
        S2["배달앱 A 인증"]
        S3["배달앱 B 인증"]
        S4["배달앱 C 인증"]
    end

    Port["🔷 SingleFlightCoordinator (Port)<br/>execute / getInflightState / forceRelease"]

    S1 & S2 & S3 & S4 --> Port

    subgraph Decorators["Decorator 합성 (안에서 밖으로)"]
        D1["① InProcess Coordinator<br/>Map + forceRelease gate + 세대 가드"]
        D2["② Deadline (Promise.race)<br/>180,000ms default"]
        D3["③ Capacity<br/>maxWaiters 30 load-shedding"]
        D4["④ Heartbeat (15s scan)<br/>long_running warn"]
        D5["⑤ Telemetry<br/>started/finished/failed/coalesced"]
    end

    Port --> D5 --> D4 --> D3 --> D2 --> D1

    D1 --> Adapter[("Backend Adapter<br/>in-process / 분산 캐시 / MQ routing")]
```

**행동 계약 7가지 — spec 헤더 명문화**

| # | 속성 | 보장 |
|---|---|---|
| 1 | 코알레싱 | 같은 키 동시 호출 → 작업 1번만 실행 |
| 2 | 결과 일관성 | 같은 cause 공유 · 스택트레이스 보존 |
| 3 | 자원 정리 | 모든 settle 경로에서 record 제거 |
| 4 | kill switch | forceRelease 후 새 owner · 세대 가드 |
| 5 | 호출자 격리 | 서로 다른 키는 영향 없음 |
| 6 | 입력 계약 | 동기 throw 도 rejection 정규화 |
| 7 | 관측성 | getInflightState 계약 노출 |
...
**JVM/Spring 매핑** — Spring `@Component` + `@Qualifier` 로 backend 어댑터 swap · Kotlin Coroutines + `Deferred` 로 in-process single-flight · Resilience4j `Bulkhead.semaphore` + `TimeLimiter` 로 deadline · MeterRegistry counter / gauge 로 telemetry.
...
**문제** — 자동 답글이 위험한 가장 큰 케이스 둘. ① 별점은 5점인데 본문은 컴플레인(sentiment 불일치 — 사장님 보복 두려움이나 라이더 평가용으로 약 30% 가 5점 + 부정 내용) ② "ㅋㅋㅋㅋㅋㅋㅋㅋ" 같은 노이즈 리뷰에 진지한 답글. AI 호출은 비싸고 느려서 모든 리뷰를 ML 에 보내면 비용·지연 손해이고, AI 모델이 "문맥 파악 불가"로 0점 반환하는 케이스도 노이즈 분류가 필요합니다. 같은 텍스트라도 길이에 따라 신뢰도가 달라야 합니다 — 100자 "조금 짰어요" 는 신뢰도 높음, 5자 "별로" 는 노이즈에 가까움.
...
**JVM/Spring 매핑** — Spring Batch `ItemProcessor` 로 노이즈 필터 · `RestTemplate` / `WebClient` 로 AI Score API · Bean Validation 으로 임계 검증 · `ApplicationEventPublisher` + `@TransactionalEventListener` 로 알림 발송 분리.

---

### 사례 14. SafeCron — 멀티 인스턴스 cron 중복 실행 차단 데코레이터 (14곳 적용) ｜ 2025.09 ~ 운영 중

**문제** — 멀티 인스턴스 환경에서 표준 `@Cron` 은 **모든 인스턴스에서 동시 실행**. 일일 결제 정합성 점검이 N개 인스턴스에서 N번 → 외부 PG API rate-limit 초과, 일일 알림톡 cron N번 → 사용자가 같은 알림톡 N개, 사용자 동기화 배치 N번 → 외부 SaaS rate-limit + 비용. 모든 cron 마다 락 처리 코드를 직접 작성하면 보일러플레이트가 폭증하고, 락 해제 누락 시 인스턴스 죽으면 락이 영구 잔존해 cron 이 영영 안 돌게 됩니다.
...
- 시니어 데코레이터 패턴 — DI 우회 + Lock acquire/release/TTL + Slack 표준화 + 환경변수 토글을 한 데코레이터에 통합.

**JVM/Spring 매핑** — Spring `@Scheduled` + ShedLock (Redis/JDBC backend) — 멀티 인스턴스 cron 분산 락 표준 패턴 · `@ConditionalOnProperty` 로 환경별 토글 · Spring AOP `@Around` 로 Slack 표준화.

---

### 사례 15. 자동 답글 종단 파이프라인 — 수집 → AI → 4단 TOCTOU 게이트 → API Key/JWT 이중 인증 게시 ｜ 2026.03

**문제** — 「사장님이 안 달아도 AI 가 자동 답글」 의 위험 4가지. ① 잘못 달면 매장 평판 직격(별점 1점에 "맛있게 드셔서 감사" 자동 답글 = 사고) ② 같은 리뷰에 두 번 게시 ③ 매장별 홍보문구 합성(쿠폰 받으세요)을 AI 답글 위 / 아래 어느 쪽? ④ 별점 컷오프(매장마다 "5점만" / "4점 이상만" 정책 다름). 추가로 DB 세션 안에서 60초 타임아웃 가능한 외부 API 호출 시 커넥션 풀이 빠르게 고갈됩니다.
...
<details>
<summary><b>📐 D1. 자동 답글 종단 파이프라인 — TOCTOU 4 게이트 + 이중 인증</b> (클릭하여 펼치기)</summary>

```mermaid
flowchart TB
    Admin[어드민 트리거<br/>auto_reply=true] --> Fetch[FETCHING_REVIEWS<br/>30건 배치]
    Fetch --> AI[AI 답글 생성<br/>MLOps Score API]
    AI --> Post

    subgraph Post[POST_PROCESSING 순서 고정]
        P1[① 리뷰 스코어링]
        P2[② 알림톡]
        P3[③ 자동 답글<br/>마지막]
        P1 --> P2 --> P3
    ...
    subgraph Gate[4단 TOCTOU 게이트 — 게시 직전]
        G1[① is_replied 플래그]
        G2[② Reply.count > 0]
        G3[③ AI 답글 기간 컷오프]
        G4[④ AI 답글 본문 존재]
    ...
    Gate -->|모두 통과| Assemble[합성 staticmethod<br/>홍보문구 · 변수 치환<br/>6 케이스 테스트]
    Gate -.->|하나라도 실패| Skip[조용히 skip<br/>silent no exception]

    Assemble --> Auth{이중 인증}
    Auth -->|API Key 시도| KeyOK[성공]
    Auth -->|401/403| JWT[JWT fallback]
    JWT --> KeyOK
    KeyOK --> BE[메인 API → 외부 플랫폼 게시]
```
...
| 계층 | 책임 |
|---|---|
| 운영 콘솔 측 | 4 단 TOCTOU 게이트로 같은 작업 안 두 번 호출 차단 (작업 자체 결함 방어) |
| 메인 API 측 | 자동답글 플래그 받아 외부 플랫폼 게시 단계 멱등성 보장 |
...
**JVM/Spring 매핑** — Spring `@Transactional(readOnly=true)` 짧은 트랜잭션 + RestClient 외부 호출은 트랜잭션 밖 분리 · Spring Security `OAuth2AuthorizedClientManager` 로 API Key → JWT fallback · Resilience4j `Retry` + `CircuitBreaker` 합성.
