...
결제 PG webhook 멱등성, 정합성 사후 탐지, 쿠폰 트랜잭션 일관성 — 「돈이 잘못 빠지면 사고」인 영역의 4중 방어 사례 모음.

### 결제 webhook 4중 멱등성 — 분산 락 (ABA-safe) · DB UNIQUE · 상태머신 · 원자 해제
...
### 결제 정합성 reconciliation — 좀비 schedule · 중복 빌링키 · admin override orphan 자동 탐지
...
### 쿠폰 멱등 적용 — 사전 검증 · 트랜잭션 · schedule fallback 보정의 3중 idempotency
...
1,000+ 매장 일괄 등록, 관계형 DB 를 큐로 쓰는 분산 작업 처리, hierarchical 동시성, 200+ 워커 물리서버 분할 운영, 본사 1명 1,000매장 한 화면 — 대량 처리 + 무중단 운영 사례.

### 매장 일괄 등록 — 비관락 X-lock + 그룹 단위 트랜잭션 경계
...
### 일괄 댓글 처리 — 관계형 DB 를 큐로 쓰는 CAS 기반 분산 작업 큐
...
### 2시간 cron 리뷰 수집 — 5,700+ 계정 × 13,000+ 매장 hierarchical 동시성
...
### 고사양 헤드리스 브라우저 워크로드 — 물리서버 분할 운영 + 무중단 배포

![고사양 헤드리스 브라우저 워크로드 — 물리서버 분할 운영 + 무중단 배포](diagrams/B5_active_active_deploy.svg)

**문제** — stealth fork 헤드리스 브라우저 30+ 워커가 6 플랫폼에서 동시에 돌면 자원 부담이 차원이 다릅니다.

- 1 워커 = 브라우저 1 인스턴스 ≈ **1 GB+ RAM**, launch 5~10초, CPU 1~2 코어 점유
- 플랫폼당 30+ 워커 × 6 플랫폼 = **동시 200+ 워커**
- 일반 클라우드 인스턴스로는 비용 폭증 + OOM 빈발 + 데이터센터 ASN 으로 분류되어 봇 탐지에 더 잘 걸림
- 봇 탐지 회피를 위해 fingerprint 다양화가 필요해 워커별 격리 필수

추가로 인증 에러 도미노 효과:

- 한 그룹 첫 매장 실패 → 봇 탐지 가속 → 같은 그룹 나머지 매장 7개도 같은 운명
- 단순 retry 는 21회 헛 호출 + 봇 탐지 더 가속

**해결** — 클라우드 ECS 가 아니라 **물리서버 + 도커 분할 운영** 으로 자원 격리 · 비용 · 봇 탐지 회피를 동시에 풀었습니다.

물리서버 사양:

- CPU **32 코어** / RAM **512 GB**
- 도커 **8 인스턴스** 로 분할 (각 4 코어 · 64 GB · blue / green / yellow / purple / orange / cyan / white / black 색명으로 식별)
- 각 도커가 독립된 fingerprint · network namespace · 자원 격리

운영 파이프라인:

- **배치 서버** (2시간 cron) 가 매장 단위로 fan-out → 동적 라우팅(Reverse Proxy)에 HTTP call
- **동적 라우팅** (`traefik/dynamic/scraper.yml`) 이 건강한 도커 인스턴스 8개로 라운드 로빈
- 라우팅 설정 파일 변경 = 즉시 라우팅 반영, 컨테이너 재기동 없이 트래픽 차단 / 복원 가능
- 한 도커 OOM / 죽음 = 나머지 7개로 자동 우회 (배치 서버 입장에서 무중단)

무중단 재시작 5-step:

1. 동적 라우팅 폴더 정리 — `.bak` 잔여물 제거 (Reverse Proxy 가 폴더 안 모든 파일 로드 → 죽은 컨테이너로 라우팅하는 사고 예방)
2. 트래픽 차단 — `scraper.yml` 비우기 (컨테이너는 살아있음 + 라우팅만 끊김, `connection refused` 가 아닌 503 응답)
3. 8 도커 동시 down + ECR pull + 순차 up
4. Health check — `/health` 120 × 5s 폴링 (최대 10분 budget · 이미지 md5 일치 확인)
5. 라우팅 복원 — `restore-traefik.sh` + HTTP 200 polling 30초 안에 8 도커 트래픽 전부 복원

인증 에러 그룹 도미노 차단:

- 한 매장 첫 인증 에러 시 `_deactivate_shops` 로 **그룹 전체** `is_active=4` 즉시 처리
- 플랫폼별 임계치 dict — 봇 탐지 심한 플랫폼은 임계치 **0** (첫 인증 오류 = 영구 장애 가정), 배달앱은 2~3 (플랫폼 특성별 동적)
- 회로 차단 패턴 — 같은 계정 추가 호출 0건 · 봇 탐지 가속 차단

**성과**

- 200+ 워커를 단일 물리서버 안에서 안정 운영 — 클라우드 동급 사양 대비 비용 절감
- 한 도커 OOM 시에도 나머지 7개로 우회 → 배치 서버 무중단
- 단일 mqscript.sh 로 8 도커 안전 무중단 재시작 — 운영 서버 md5 일치 확인
- 봇 탐지 회피 — 첫 매장 실패 시 그룹 전체 차단, 같은 계정 추가 요청 **0건**
- 봇 탐지가 가장 심한 플랫폼 worker 14 → 30 → 25 안정화

**JVM/Spring 재설계** ([commerce-external-gateway-kt](https://github.com/PreAgile/commerce-external-gateway-kt))

- Spring Boot Actuator `/health` + readinessProbe / livenessProbe + Micrometer 메트릭
- Resilience4j `Bulkhead` (Semaphore vs FixedThreadPool) — 외부 의존성별 풀 분리
- Resilience4j `CircuitBreaker` 임계치 dict 패턴 — `ADR-007` 50%/70%/90% 측정 후 70% 채택
- K8s rolling update + maxSurge/maxUnavailable 또는 동등 패턴 (배치 서버 무중단)

---

### BIZ 대시보드 1,000매장 × 7일 — 2단계 쿼리 + DB-level pagination + SQL VIEW SSOT
...
6개 외부 플랫폼 통합에서 발생하는 세션 race · IP 평판 · Akamai 봇 탐지 · single-flight coalescing 을 분산 락 · fencing token · 상태 머신 · Port-Adapter 로 풀어낸 사례.

### 세션 락 레지스트리 — FIFO 큐 + lease TTL + cold-start guard 로 30+ worker 동시 로그인 race 직렬화
...
### 자체 IP 평판 시스템 — 외부 의존 제거 + 비용 88.75% 절감 (월 800만 → 90만)
...
### Akamai Bot Manager 우회 — 인증 쿠키 상태 머신 · Referrer Warming · Click Loop (77.8% → 100%)
...
### Single-Flight Coordinator 포트화 — Hexagonal + Decorator 5종 합성, 7가지 행동 계약 spec 명문화
...
AI 모델을 운영에 안전하게 끼우는 노이즈 필터 · 길이 가중치 · TOCTOU 게이트, 멀티 인스턴스 cron 표준화 데코레이터 — AI · 자동화 영역의 운영 안전망 사례.

### 부정 리뷰 탐지 — 6신호 노이즈 필터 + 길이 가중치 + 별점·스코어 OR 분류
...
### SafeCron — 멀티 인스턴스 cron 중복 실행 차단 데코레이터 (14곳 적용)
...
### 자동 답글 종단 파이프라인 — 수집 → AI → 4단 TOCTOU 게이트 → API Key/JWT 이중 인증 게시
...
5년 동안 Node.js 로 운영하며 부딪힌 **분산 락 · 결제 멱등성 · 트랜잭션 분리 · 외부 의존성 격리** 문제를 Java/Kotlin/Spring 환경에서 동일한 시나리오로 다시 풀어보고, 결정 과정을 **ADR (Architecture Decision Record — 설계 결정과 그 근거 · 대안 · 결과를 코드 저장소에 함께 박는 문서)** 로 남기는 프로젝트입니다. 현재까지 **ADR 49건, 실험 47건, 기술 블로그 17편** 누적.
