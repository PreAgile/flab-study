...
> 아래 토글들은 **②단계 곳곳에 숨은 OS·네트워크 기초**를 펼친 것 — 1번의 IME부터, 5번의 byte stream/wire·syscall·socket buffer·캡슐화·fd까지 한 줄기로 이어진다.

<details>
<summary>📌 <b>IME(입력기)란? — 클릭해서 펼치기</b></summary>

**IME = Input Method Editor(입력기).** 키 하나로 못 치는 문자를 키 입력 조합으로 만들어내는 **OS 레벨 소프트웨어**(macOS 입력기, Windows IME, Linux IBus/fcitx) — 브라우저가 아니다.

**왜 필요한가** — 키보드는 키가 ~100개뿐인데 한글 음절은 11,172자, 한자는 수만 자. 글자마다 키를 둘 수 없으니 IME가 **조합(composition)**으로 만들어낸다.

한글 조합 — `김`이 만들어지는 길 (stateful 버퍼):
```
키 입력    IME 조합 버퍼          codepoint
 ㄱ   →   [ㄱ]      (초성)
 ㅣ   →   [기]      (초성+중성 조합, "조합 중" 밑줄 상태)
 ㅁ   →   [김]  →  확정 → U+AE40
```
그래서 ②번 1단계에서 "브라우저 메모리에 도달했을 땐 이미 codepoint"인 이유 — **조합은 IME가 이미 끝냈고, 브라우저는 완성된 codepoint를 받아 encode(2단계)만 한다.**

**운영 관점 — IME가 만드는 두 가지 단골 버그**:
- **조합 중 이벤트 오발**: 웹에서 `keydown`/`input`은 조합 도중에도 발생한다. 그래서 검색어 자동완성·React 제어 컴포넌트가 미완성 자모(ㄱ)에 반응해 **글자 누락·중복 입력**이 생긴다. → `compositionstart`/`compositionupdate`/`compositionend` 이벤트로 "조합이 끝났을 때만" 처리해야 한다.
- **NFC vs NFD 정규화**: 한글은 Unicode에 **완성형(NFC, `김`=U+AE40 단일)**과 **조합형(NFD, ㄱ+ㅣ+ㅁ 분해)** 두 표현이 있다. **macOS 파일시스템은 NFD, 대부분 시스템은 NFC** → 같은 "김"이 다른 codepoint sequence → 다른 UTF-8 byte → 다른 percent-encoding → **파일명·검색·URL 불일치**(macOS↔Linux 단골 사고). 문서의 charset 깨짐 주제와 직결.
...
<details>
<summary>📌 <b>OSI 7계층 vs TCP/IP 4계층 — 그래서 TLS는 몇 계층? — 클릭해서 펼치기</b></summary>

위 표는 **OSI 7계층** 기준이다. 그런데 **실제 인터넷이 돌아가는 모델은 TCP/IP 4계층** — 둘은 1:1이 아니다. TCP/IP가 먼저 *돌아가는 코드*로 인터넷을 지배했고(1970s ARPANET), OSI는 나중에 위원회가 만든 더 "올바른" 참조 모델이라 **용어로만 살아남았다**(우리가 "L7"·"L4"라 부르는 게 그것).

매핑 — OSI의 **위 3개가 하나로 접힌다**:
```
OSI 7계층                  TCP/IP 4계층           실제 프로토콜
──────────────────────────────────────────────────────────────
L7 Application  ┐
L6 Presentation ├────────▶  Application          HTTP, TLS, DNS, gRPC
L5 Session      ┘            (응용)
──────────────────────────────────────────────────────────────
...
──────────────────────────────────────────────────────────────
L2 Data Link    ┐
L1 Physical     ┴────────▶  Link(Network Access) Ethernet, WiFi, MAC
```

**왜 접히나** — TCP/IP는 Session(L5)·Presentation(L6)을 독립 계층으로 안 만들고 **앱 프로토콜 안으로 흡수**했다. 암호화는 TLS가, 인코딩은 앱이(UTF-8·JSON), 압축은 gzip이 — "표현 계층 서비스"라는 게 따로 없다. 그래서 **TCP 입장에선 L4 위가 전부 정체 모를 payload byte 덩어리**(그게 TLS인지 HTTP인지 모르고 알 필요도 없음).

**그래서 TLS는 몇 계층?** — OSI로는 "암호화니 L6" "세션 만드니 L5" 논쟁 대상이지만, **TCP/IP 4계층엔 그 칸이 없어 그냥 Application**이다. wire엔 L5·L6 헤더 없이 중첩만 있다:
```
[ Eth | IP | TCP | [ TLS record [ HTTP ] ] ]
                    └── TCP는 [TLS[HTTP]] 전체를 그냥 payload로만 봄 ──┘
```

> **4계층 vs 5계층 혼동**: RFC 1122 공식 모델은 **4계층**(Link·Internet·Transport·Application), Kurose 등 교재는 종종 **5계층**(L1·L2를 Physical·Link로 분리). 차이는 *맨 아래를 쪼개냐*일 뿐, **위쪽 L5/L6/L7→Application 합침은 동일**. 면접에선 둘 다 맞다.
