...
│ ②  텍스트→Unicode→UTF-8→percent-encoding (한글이 ASCII byte 되는 길)│ L6
...
**한 단락 narrative**:
사용자가 주소창에 한글이 섞인 URL을 친다. 브라우저가 그 텍스트를 Unicode codepoint로 분해하고 UTF-8 byte로 직렬화한 뒤 percent-encoding으로 ASCII-safe URL을 만든다. hostname은 DNS 4계층 cache hierarchy를 거쳐 IP가 되고, 클라이언트는 그 IP를 향해 TCP 3-way handshake로 연결을 맺고 TLS 1.3으로 비밀과 신원을 협상한다. IP packet은 라우터 hop을 타고 인터넷을 가로지른다. 이때 IP는 처음부터 끝까지 그대로지만 MAC은 hop마다 바뀐다. 도착 직전 Load Balancer가 SSL을 풀고 backend 하나를 고른다. Nginx worker가 epoll로 그 connection을 받아 upstream keepalive pool로 Tomcat에 던지고, Tomcat의 Acceptor→Poller→Executor 3단 파이프라인이 byte stream을 HttpServletRequest로 변환한다. Spring controller가 HikariCP에서 DB connection을 빌려 JDBC PreparedStatement로 SQL을 보내고, DB가 Parser→Optimizer→Executor→Storage를 거쳐 row를 돌려준다. 그리고 응답은 정확히 같은 길을 역순으로 풀린다.
...
HTTP는 본질적으로 ASCII protocol이다. RFC 9112가 정의하는 request-line/header/URL은 모두 ASCII printable(0x21~0x7E) 기반. 한글 같은 비-ASCII byte는 HTTP 메시지에 그대로 못 들어간다. 그래서 브라우저는 path/query의 비-ASCII 문자를 **5단계 변환**으로 ASCII-safe하게 바꾼다.

`김면수`라는 3글자가 wire에 도달하는 길:

1. **텍스트 → Unicode codepoint**: 김=U+AE40, 면=U+BA74, 수=U+C218 (3 codepoint).
2. **codepoint → UTF-8 byte**: 한글 음절 영역은 3 byte 패턴 → `EA B9 80 EB A9 B4 EC 88 98` (9 byte).
3. **byte → percent-encoding**: 각 byte를 `%HH`로 → `%EA%B9%80%EB%A9%B4%EC%88%98` (27 ASCII char).
4. **HTTP 메시지 조립**: `GET /users/%EA%B9%80%EB%A9%B4%EC%88%98 HTTP/1.1\r\nHost: example.com\r\n\r\n`.
5. **byte stream → TCP segment**: 이미 ASCII라 추가 변환 없이 그대로 socket에 write.

서버는 정확히 역순으로 푼다. percent-decode → UTF-8 decode → Java String. 어느 한 단계라도 charset 합의가 어긋나면 그 지점에서 글자 깨짐.
...
[Unicode]       U+AE40  U+BA74  U+C218
       │
       │ ② codepoint → UTF-8 byte (RFC 3629, 한글은 3 byte 패턴)
       ▼
[UTF-8 byte]    0xEA 0xB9 0x80  0xEB 0xA9 0xB4  0xEC 0x88 0x98   (9 byte)
                ── 김 ──        ── 면 ──        ── 수 ──
       │
       │ ③ byte → percent-encoding (RFC 3986)
       ▼
[percent-enc]   %EA%B9%80%EB%A9%B4%EC%88%98                       (27 ASCII char)
       │
       │ ④ HTTP 메시지 조립
       ▼
[HTTP request]  GET /users/%EA%B9%80%EB%A9%B4%EC%88%98 HTTP/1.1\r\n
                ...
                \r\n
       │
       │ ⑤ byte stream → TCP segment (이미 ASCII라 추가 변환 없음)
       ▼
...
브라우저가 encode, 서버가 decode. 양쪽 charset 합의가 어긋나면 그 경계에서 글자 깨짐. Spring Boot 환경에서 charset 계약은 3+ 지점:
...
> HTTP는 ASCII protocol이라 비-ASCII byte를 그대로 못 실어요. `김`은 Unicode U+AE40인데 UTF-8로 직렬화하면 3 byte `0xEA 0xB9 0x80`이 됩니다. 각 byte를 `%HH` 형식으로 escape해서 `%EA%B9%80`. ASCII-safe해진 URL이 HTTP request line에 박혀 wire로 흐르고, 서버는 percent-decode → UTF-8 decode 역순으로 String을 복원합니다. 30년 묵은 middlebox 호환성 때문에 ASCII invariant를 깰 수 없는 게 본질적인 이유.
...
> 12단계 — 브라우저 URL 파싱 + 캐시/HSTS 체크 → 한글이 있으면 UTF-8 byte → percent-encoding → DNS 4계층으로 hostname → IP → TCP 3-way + TLS 1.3 handshake → IP packet이 라우터 hop을 타고 인터넷 횡단(IP는 그대로, MAC은 hop마다 갱신, TTL 감소) → Load Balancer에서 SSL termination + backend 선택 → Nginx worker가 epoll로 받아 upstream keepalive pool로 Tomcat에 전달 → Tomcat Acceptor→Poller→Executor 3단 파이프라인이 byte를 HttpServletRequest로 변환 → Spring controller → HikariCP에서 DB connection 빌려 PreparedStatement로 SQL → DB가 Parser/Optimizer/Executor/Storage 거쳐 row 반환 → 응답이 정확히 역순으로 풀린다. 모든 단계에서 OSI 7계층이 동시에 동작하며 헤더를 입히고 벗긴다.

### Q2. 한글이 URL에 그대로 못 들어가는 이유는?

> HTTP/URL이 ASCII protocol(7-bit)이라서. 1990년대 7-bit ASCII가 사실상 표준이던 시대에 태어났고, 30년 묵은 middlebox·proxy·WAF·log analyzer가 그 invariant에 hard-code되어 있어요. URL은 HTTP 밖(HTML, email, Slack, log)으로도 새어 나가는데 그 모든 환경에서 안 깨지려면 ASCII-only가 필요합니다. 그래서 한글은 UTF-8 byte로 직렬화 → percent-encoding(`%HH`) ASCII-safe 변환. `김`이 `%EA%B9%80`이 되는 이유.
