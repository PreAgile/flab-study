...
> 다음 5개 주체가 차례로 일한다 — **IME가 map** → **브라우저 UTF-8 인코더가 encode** → **URL parser가 escape** → **HTTP client가 serialize** → **kernel TCP/IP가 segment 조립**.
...
| 동사 | 의미 | 사용 위치 |
|---|---|---|
| **map** | lookup-table 1:1 매핑 | IME가 키 입력 → codepoint |
...
```
[키 입력 "ㄱ+ㅣ+ㅁ"]
     │ ① OS의 IME가 매핑(map) — macOS 한글입력기 / Windows 한글IME
