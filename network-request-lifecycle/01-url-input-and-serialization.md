...
> 서버는 정확히 역순으로 복원한다. **한 단계라도 charset이 어긋나면 그 지점이 글자 깨짐의 원인.**
...
서버는 정확히 역순. percent-decode → UTF-8 decode → String 객체로 복원. 각 단계의 책임 주체가 다르고, 어느 한 곳에서 charset 가정이 어긋나면 즉시 글자 깨짐.
...
| **charset 결정** | 브라우저 자동 (UTF-8) | **Tomcat `URIEncoding=UTF-8`** (path/query) + **`CharacterEncodingFilter`** (POST body) | 글자 깨짐 |
...
**책임 분리의 본질** — 각 계층이 자신의 charset 계약을 명시해야 한다. 한 계층의 가정이 어긋나면 그 지점에서 글자 깨짐.
...
5. **"한글 검색이 random하게 글자 깨짐. 진단 절차는?"**
...
- [ ] 글자 깨짐 7대 패턴 진단표를 보고 각 증상의 원인 계층(어디서 charset이 어긋났는지)을 짚을 수 있다.
