# 09. Mock Interviews — 마인드맵으로 정리한 면접 꼬리질문 트리

> **JVM 면접의 모든 질문은 6개 주제 클러스터로 압축된다.**
> 면접관이 어떤 질문을 던져도 "이건 가지 N의 질문이다"라고 분류 → 그 가지의 키워드 3개 순서로 답변 → 듣는 사람의 관심에 따라 인접 가지로 확장.
> 표면(개념) → 깊이(내부 동작) → 운영(진단/해결) → 함정(트레이드오프) 4단으로 꼬리에 꼬리를 무는 구조.
...
2. **1~6장을 순서대로 학습한다** — 각 장이 마인드맵의 한 가지에 정확히 대응. 모든 Q&A는 토글로 감춰뒀다. 먼저 자기 입으로 답해본 뒤 펴라.
3. **7장 레벨별 종합 면접**으로 시야 통합.
4. **8장 1시간 시뮬레이션**으로 실전 흐름 점검.
...
> **"JVM 면접의 모든 질문은 6개 주제 클러스터로 압축된다. 각 가지마다 표면 → 깊이 → 운영 → 함정 4단계로 꼬리에 꼬리를 문다."**
...
```
   ...
   ┌─────────┬─────────┬──────┴──────┬─────────┬─────────┐
   │         │         │             │         │         │
  ① 메모리   ② 실행    ③ Class       ④ 동시성  ⑤ 버전    ⑥ OOP
   & GC     엔진      Lifecycle      Threading 진화      설계
   │         │         │             │         │         │
   │     ┌───┼───┐  ┌──┼──┐       ┌──┼──┐  ┌──┼──┐   ┌──┼──┐
  Heap  JIT  AOT   CL  Linking  JMM  VT   8→11  17→ SOLID Clean
  Stack Tiered Graal Init Unload sync  Pinning  21 LTS  DDD  FP
  GC   EA  Deopt  CP/Resolution  Lock  Loom    JEP   MSA  Test
```
...
|---|---|---|---|
| **① 메모리 & GC** | 메모리 영역 6종 | Young/Old + GC 알고리즘 | 누수 진단 (MAT/NMT) |
| **② 실행 엔진** | Interpreter → C1 → C2 | Inlining / EA / Deopt | AOT (GraalVM Native) |
| **③ Class Lifecycle** | ClassFile / CP / Linking | ClassLoader 위임 / 격리 | Init lazy / Unload |
| **④ 동시성** | JMM / happens-before | synchronized / Lock 종류 | Virtual Thread / Loom |
| **⑤ 버전 진화** | 8 → 11 → 17 → 21 LTS | JPMS / GC 변천 | VT / Pattern matching |
| **⑥ OOP 설계** | SOLID (특히 DIP) | DDD / Clean Architecture | OOP × FP 혼용 |

### 면접 답변 흐름

> 면접관 질문 → 루트 문장 → 6개 가지 중 진입 가지 선택 → 그 가지의 키워드 3개 순서로 → 표면→깊이→운영→함정 깊이 단계 → 듣는 사람의 관심에 따라 인접 가지로
...
> "JVM의 메모리 구조와 GC가 그 위에서 어떻게 동작하는지 설명하라."

### 1.2 키워드 1 — 메모리 영역 6종 (Ch 02)
...
#### A1. 🟡 JVM의 메모리 구조와 각 영역의 역할, GC가 각 영역에 어떻게 작동하는지 상세히 설명해보세요.
...
#### A2. 🟡 GC의 Young/Old Generation 차이와 각각의 GC 알고리즘 동작 방식은?
...
| `-XX:NewRatio` / `-Xmn` | Young/Old 비율. Young 크면 Minor GC 길지만 promotion 적음. 작으면 promotion 빨라져 Old 부담 큼. |
...
> "Java가 어떻게 빠른가? bytecode가 native가 되기까지의 길과 그 위의 최적화는?"

### 2.2 키워드 1 — Interpreter → C1 → C2 (Tiered)
...
#### B1. 🟡 JIT vs AOT 컴파일러의 차이, HotSpot JIT 최적화 기법(인라이닝, 루프 언롤링 등)은?
...
#### B2. 🟡 JIT 옵티마이저 단계별 동작 원리와 실전에서의 튜닝 포인트는?
...
#### B4. 🟡 JIT의 프로파일링 데이터 수집 방식과 실제 코드 최적화에 미치는 영향은?
...
#### B6. 🟡 JIT의 Deoptimization(역최적화) 상황과 그 원인은?
...
#### B7. 🟡 JVM에서 JIT의 동작을 실시간으로 모니터링하는 방법은?
...
### 2.4 키워드 3 — AOT (GraalVM Native Image)
...
## 3. 가지 ③: Class Lifecycle & ClassLoader

### 3.1 핵심 질문

> ".class 파일이 메모리에 올라와 실행 가능해질 때까지 무엇이 일어나는가? 누가 로드하나?"

### 3.2 키워드 1 — ClassFile & ConstantPool & Linking
...
### 3.4 키워드 3 — Initialization lazy & Unload
...
> "여러 스레드가 동시에 메모리를 만지는데 어떻게 일관성과 진행을 보장하나? Loom 이후엔 어떻게 바뀌었나?"

### 4.2 키워드 1 — JMM / happens-before
...
|---|---|---|
| 가시성 | yes | yes |
| atomicity | no (단일 read/write만) | yes |
...
### 4.3 키워드 2 — synchronized / Lock 종류
...
#### E1. 🟡 JVM에서 스레드 관리 및 동시성 제어 방식과 실무 동시성 이슈 및 해결 경험은?
...
#### E5. 🟡 java.util.concurrent의 주요 클래스별 동작 원리와 실전 적용 사례는?
...
#### E6. 🟡 데드락, 라이브락, 스타베이션 등 동시성 장애의 실전 사례와 해결 전략은?
...
#### E2. 🟡 JVM 스레드 스케줄링 방식과 OS 레벨 스레드와의 차이는?
...
#### E7. 🔴 Fork/Join 프레임워크와 Parallel Stream의 내부 동작 차이와 실무 적용 고려사항은?
...
> "Java 8 → 11 → 17 → 21 LTS를 거치며 무엇이 어떻게 바뀌었고, 그게 시스템 설계에 어떤 영향을 줬는가?"

### 5.2 키워드 1 — LTS 흐름 (8 → 11 → 17 → 21)

#### C1. 🟡 자바 8, 9, 11, 17, 21 버전에서의 주요 변화와 실무 시스템 설계에 미치는 영향은?
...
#### C6. 🟡 LTS 버전 선택 전략과 실제 서비스 운영에서의 고려사항은?
...
### 5.3 키워드 2 — JPMS / GC 변천 / Deprecated
...
### 5.4 키워드 3 — Virtual Thread 도입의 영향

#### C8. 🔴 최신 자바 버전의 Virtual Thread(Loom) 도입이 서버 아키텍처에 미치는 영향은?
...
> "SOLID, DDD, Clean Architecture 같은 원칙이 실제 코드와 시스템에 어떻게 작용하나?"

### 6.2 키워드 1 — SOLID (DIP 중심)

#### D1. 🟡 SOLID 중 본인이 가장 중요하게 생각하는 원칙과 실제 적용 경험은?
...
#### D6. 🟡 SOLID를 적용한 리팩토링 전후의 코드 비교와 성능/유지보수성 변화는?
...
### 6.3 키워드 2 — DDD / Clean Architecture / MSA
...
## 8. 1시간 종합 시뮬레이션 (Part 3)

| 시간대 | 주제 | 관련 가지 |
|---|---|---|
| 0-10분 | Self-intro + 기본: "JVM 메모리 영역을 그려보세요" / "GC 알고리즘 하나 깊이" | ① 메모리 & GC |
| 10-30분 | Deep dive: "JIT 컴파일 흐름 + Tiered" / "Virtual Thread 동작 원리" | ② 실행 엔진, ④ 동시성 |
| 30-50분 | Production: "P99 latency spike 진단 절차" / "Container OOM-killed 분석" | ① + Ch 10 ops 시나리오 |
| 50-60분 | 시스템 설계: "100대 서비스의 JVM 표준화 전략" | Principal 종합 (7.3) |
...
- [ ] 가지 ① 메모리 & GC: 6개 영역 그림 + G1/ZGC 동작 + OOM 진단 5단계
- [ ] 가지 ② 실행 엔진: Tiered 5단계 + EA/Deopt + AOT 한계
- [ ] 가지 ③ Class Lifecycle: CP 5단계 흐름 + ClassLoader 위임 + lazy init
- [ ] 가지 ④ 동시성: JMM happens-before + Lock 3종 + VT/Pinning
- [ ] 가지 ⑤ 버전 진화: 8→11→17→21 LTS 핵심 변화 + 마이그 체크리스트
- [ ] 가지 ⑥ OOP: DIP의 본질 + SOLID 충돌 트레이드오프 + DDD/Clean의 OOP 매핑
- [ ] Junior/Senior/Principal 종합 15문 막힘없이 답한다
- [ ] 1시간 시뮬레이션 흐름(표면 → 내부 → 운영 → 결정)을 실전처럼 시연한다
