...
**보조 박스 ─ 측정한 데이터는 어디 쌓이나? = MDO (Method Data Object)**

> **코드는 Code Cache, 데이터는 MDO** — 둘이 다른 곳에 산다.
>
> ```
> Profiled segment의 native code              MDO (Method 객체의 별도 필드)
> ─────────────────────────────────           ────────────────────────────
> inc [MDO + branch_taken_offset]      ─→     branch_taken_count = N
> inc [MDO + type_profile[ArrayList]]  ─→     type_profile {
>   ...                                          ArrayList:  9999,
>                                                LinkedList: 1
>                                              }
>                                              invocation_count = M
...
> | 구분 | 무엇 | 어디 |
> |---|---|---|
> | **measurement instrumentation** | 측정 *명령* (inc 등) | Code Cache의 Profiled segment |
> | **profile data** | 측정 *결과* (통계 값) | **MDO** — Method 객체의 별도 자료구조 |
>
> **MDO = Method Data Object** (`MethodData` in HotSpot 소스)
> - 각 Method 객체마다 한 개씩 붙어 있는 통계 저장소.
> - 위치: **Metaspace** 영역 (Method 객체와 인접한 자료구조).
> - 내용: 호출 횟수, branch taken/not_taken, 가상 호출의 receiver 타입 분포, null 발생 여부, 등.
> - **C1 tier 2/3가 생산자** — 측정 명령으로 MDO 갱신.
> - **C2가 소비자** — MDO를 입력으로 읽어 speculative 최적화 결정.
>
> → 토글 ⑦의 "C2가 profile을 소비"는 사실 **"C2가 MDO를 읽어 가정으로 받아들임"**의 다른 표현. **MDO가 C1 → C2를 잇는 통신 채널**.
...
> **보조 박스 ─ Code Cache는 JVM 메모리에서 어느 파트인가?**
>
> Heap도, JVM Stack도, Native Method Stack도 아닌 **JVM 프로세스 가상 메모리 안의 독립된 별도 영역**. Heap·Stack·Metaspace와 **형제 관계**.
>
> ```
> JVM 프로세스 가상 메모리 (OS가 준 주소 공간)
> ─────────────────────────────────────────────
> │
> ├─ Heap                       │ 객체, 배열
> │   -Xmx                       │ GC 관리
> │
> ├─ Metaspace (JDK 8+)         │ Class 메타, Method 객체, MDO
> │   -XX:MaxMetaspaceSize       │ ClassLoader 단위 회수
> │
> ├─ Thread별 영역 (N개 thread)
> │   ├─ JVM Stack              │ Java 메서드 stack frame
> │   │   -Xss                   │ (operand stack, local var)
> │   ├─ PC Register             │ 현재 bytecode 위치
> │   └─ Native Method Stack     │ JNI 호출 시 C/C++ stack
> │
> ├─ ★ Code Cache               │ ← 여기!
> │   -XX:ReservedCodeCacheSize  │ JIT가 만든 native 명령어
> │   (PROT_EXEC, W^X 보안)       │ Non-method + Profiled + Non-profiled
> │
> ├─ Direct Memory (off-heap)   │ NIO ByteBuffer, Netty
> │   -XX:MaxDirectMemorySize    │ Heap 밖
> │
> ├─ GC bookkeeping             │ Card Table, RSet, Mark Bitmap
> └─ JNI/Native memory          │ JNI lib이 malloc한 영역
> ```
>
> **Code Cache의 결정적 차별점 = PROT_EXEC** (실행 가능 플래그).
>
> | 영역 | 들어가는 것 | PROT_EXEC | 관리자 |
> |---|---|---|---|
> | Heap | 객체, 배열 | ✗ | GC |
> | Metaspace | Class 메타, Method, MDO | ✗ | ClassLoader |
> | JVM Stack | 메서드 frame | ✗ | thread 수명 |
> | **Code Cache** | **JIT native 명령어** | **✓** | **Code Sweeper** |
> | Direct Memory | NIO buffer | ✗ | 명시적 해제 |
>
> → **나머지 영역은 다 데이터, Code Cache만 실행 가능한 메모리**. CPU의 instruction pointer가 직접 진입 가능. 이게 별도 영역인 본질적 이유의 가장 명확한 표현.
...
> **보조 박스 ─ Back-edge Counter와 OSR 깊게**
>
> **"백워드 점프(backward jump)"란?**
>
> 코드는 보통 위→아래로 흐른다(forward). 루프는 끝에서 시작점으로 **뒤로** 점프 — bytecode 명령 주소가 현재보다 **작은 곳으로 점프**.
>
> ```java
...
> bytecode 주소
>   2: iload_1         ← 루프 시작
>   5: if_icmpge 19    ← forward jump (조건 안 맞으면 19로 탈출)
>  12: iinc 1, 1       // i++
>  15: goto 2          ← ★ backward jump! 15 → 2 (뒤로) = back-edge
>  19: return
> ```
>
> 인터프리터 stub의 `goto` 처리부에 "**target_pc < current_pc면 back-edge counter += 1**"이 박혀 있음. → 루프 한 번 돌 때마다 +1.
>
> **"Back-edge"라는 이름의 유래**
>
> 컴파일러 이론의 **CFG(Control Flow Graph)** 에서 "이미 방문한 노드로 돌아가는 엣지"가 back edge. 모든 루프는 반드시 하나 이상의 back edge를 가짐 → **back-edge counter = 루프 횟수 counter**의 본질.
>
> ```
>        [block A]
>           ↓ forward edge
...
>        [block C]
>           │
>           └─ back edge ─→ [block A]   ← 루프의 본질
> ```
>
> **왜 별도 counter가 필요한가?**
>
> Invocation counter만으로 못 잡는 경우:
> ```
> processOneBillion() {
>     for (int i = 0; i < 1_000_000_000; i++) { ... }   // 10억 번 루프
> }
> ```
> - 호출 한 번 → invocation counter는 +1만 올라감
> - 한 호출 안에서 10억 번 돌고 끝 → invocation counter 임계 못 넘김
> - → 영원히 인터프리터로 실행 → **50배 손해**
>
> Back-edge counter가 해결:
> - 루프 한 바퀴마다 +1 → 임계(예: 10,000) 도달
> - "지금 실행 중인 이 메서드를 컴파일하라" 신청
>
> **두 counter의 분업**
> - **Invocation counter** = 자주 호출되는 **작은 메서드** 잡음 (web handler 등)
> - **Back-edge counter** = 한 번 호출에 오래 도는 **큰 메서드** 잡음 (batch, data processing)
> - → 두 counter가 JVM의 모든 hot 패턴 커버.
>
> **OSR (On-Stack Replacement) — back-edge 컴파일이 끝났을 때의 문제 해결**
>
> 일반 컴파일은 "다음 호출부터 native"라 OK. 그런데 back-edge로 트리거된 컴파일은:
> ```
> processOneBillion() 호출 → 인터프리터 시작
> 5천만 번 루프 → back-edge 임계 → enqueue
> 백그라운드 컴파일 완료...
>    ★ 하지만 지금 stack frame은 이미 인터프리터 frame
>    ★ "다음 호출"이라는 게 없음 — 이 호출이 끝나면 메서드도 끝
>    ★ 남은 5억 번이 그대로 인터프리터로 끝남? → 손해
> ```
>
> **OSR이 해결**: 실행 중인 인터프리터 frame을 **같은 위치의 native frame으로 현장 교체**.
>
> ```
> [루프 5천만 번 돈 시점, 컴파일 완료]
> 인터프리터 frame:               OSR로 교체된 native frame:
> ─────────────                   ───────────────────────
> i = 50,000,000      ──→         %rax = 50,000,000  (register로)
> sum = 1.2e15        ──→         %rbx = 1.2e15
> operand stack       ──→         (operand stack 없음, register만)
> PC: 루프 안 어딘가  ──→         OSR entry point (그 bytecode index)
> ```
>
> safepoint에서 인터프리터 frame을 헐고, 같은 상태를 native frame으로 재구성해 **OSR entry**(특수 진입점)로 점프. 살아 있는 변수를 register/stack에 옮기는 게 핵심.
>
> **두 컴파일 비교**
>
> | | 일반 컴파일 | OSR 컴파일 |
> |---|---|---|
> | 트리거 | Invocation counter 임계 | **Back-edge counter** 임계 |
> | 진입점 | 메서드 시작 (entry) | **루프 중간** (특정 bytecode index) |
> | 효과 시점 | **다음 호출**부터 | **현재 실행 중**인 frame을 교체 |
> | nmethod 형태 | 정상 | "OSR entry"라는 특수 진입점 포함 |
> | 비용 | 일반 | **현재 frame state 복원** 필요 → 비쌈 |
>
> **시니어 관점 ─ 어디서 만나나?**
>
> - **batch job, ETL, 데이터 처리** — 한 호출에 천만 번 도는 hot path. OSR 없으면 인터프리터로 다 처리 = 50배 손해.
> - **PrintCompilation의 `%` 플래그** (가지 ⑤):
>   ```
>      142    3 %   4       MyApp::process @ 12 (123 bytes)
>                  ↑                            ↑
>                  OSR 표시                     OSR 진입 bci (bytecode index)
>   ```
>   `%`가 보이면 = back-edge counter 트리거의 결과 = 큰 루프가 hot이 됨.
>
> **한 문장**
> > "**Back-edge = 루프의 backward jump**(CFG에서 이미 방문한 노드로 돌아가는 엣지). 인터프리터가 backward jump 만날 때마다 counter +1. 임계 도달 시 **메서드 실행 중에 컴파일 신청** → **OSR로 인터프리터 frame을 native frame으로 현장 교체** → 남은 루프 반복은 native 속도로. **Invocation counter는 작고 자주 호출되는 메서드를, Back-edge counter는 한 번 호출에 오래 도는 메서드를** 잡음."
