...
## 🏢 왜 JVM을 깊이 학습하나 — 운영 현장의 관점

> 이 가이드의 진짜 목적은 **JVM 명세를 외우는 것이 아니다**.
> prod에서 P99 latency가 2초로 튀었을 때, OOM-killed가 났을 때, Full GC가 매분 발생할 때 — 그 원인을 **분석하고 해결**할 수 있는 사고력을 만드는 것이다.

### 실제 기업이 JVM을 바라보는 시각

| 회사 관점 | 무엇을 본다 |
|---|---|
| **SRE/플랫폼팀** | 컨테이너 limit vs JVM 실제 RSS, GC pause 분포, 자동 스케일 트리거 |
| **백엔드 개발자** | P99 latency, GC 영향, ClassLoader 누수 (특히 hot deploy 환경) |
| **DevOps** | 컨테이너 이미지 크기 (jlink/Native), 시동 시간 (콜드스타트), 메모리 효율 |
| **성능 엔지니어** | JIT 최적화, Inline cache, Escape Analysis, allocation hot path |
| **금융/거래 시스템** | STW pause < 1ms (ZGC/Shenandoah), tail latency (P99.9, P99.99) |

→ **"JVM 명세를 안다" ≠ "JVM 장애를 고친다"**. 두 번째가 진짜 가치.

### 흔히 마주치는 운영 장애 7대 카테고리

이 가이드의 모든 챕터가 결국 답하려는 질문:

```
1. P99 latency가 갑자기 튄다 (GC pause? TTSP? JIT 컴파일?)
   → 챕터 04 (GC), 03 (Execution Engine), 10 (Ops Scenarios)

2. OutOfMemoryError: Java heap space
   → 챕터 02 (Heap), 04 (GC), 10 (Ops Scenarios) — heap dump + MAT

3. OutOfMemoryError: Metaspace (ClassLoader 누수)
   → 챕터 01 (Class Lifecycle), 02 (Metaspace), 10 (Ops Scenarios)

4. Container OOM-killed (호스트 메모리는 여유 있는데 죽음)
   → 챕터 02 (Runtime Data Areas), 7단 측정·진단

5. Full GC 매분 발생 (premature promotion / Old gen 압박)
   → 챕터 04 (GC), 10 (Ops Scenarios)

6. JIT 컴파일이 멈췄다 (Code Cache full)
   → 챕터 03 (Execution Engine), 10 (Ops Scenarios)

7. Virtual Thread pinning / synchronized + native
   → 챕터 05 (Threading), 10 (Ops Scenarios)
```

### 모든 챕터에 적용되는 "실전 사례 통합" 원칙 ⭐

각 챕터의 **6단 (트레이드오프) + 7단 (측정·진단)** 는 단순 옵션 나열이 아니라 **실제 운영 시나리오와 연결**되어야 한다:

- ❌ "`-XX:MaxGCPauseMillis=200`은 G1의 STW 목표 설정 옵션이다." (사실 나열)
- ✅ "Spring Boot 앱이 P99 spike를 겪을 때 `-XX:MaxGCPauseMillis=200`로 조정 → 측정 결과 P99 800ms → 200ms 개선. 다만 처리량 5% 감소 트레이드오프." (사례 + 측정 + 트레이드오프)

각 챕터의 7단에 다음 4요소 포함을 권장:
1. **증상 패턴** — prod에서 어떻게 보이나
2. **진단 명령** — 어떤 도구로 어떻게 분석
3. **해결 패턴** — 일반적 조치
4. **공개 사례 참조** — 신뢰할 만한 출처 (Netflix tech blog, LinkedIn engineering, OpenJDK JEP, 컨퍼런스 발표 등)

→ 상세 사례집은 [10-ops-scenarios](./10-ops-scenarios/) 챕터에서 종합.
→ **빅테크 8대 운영 장애 패턴 + 진단/해결**: [10-ops-scenarios/00-real-world-cases.md](./10-ops-scenarios/00-real-world-cases.md) ⭐

### 참조 — 국내/해외 빅테크 기술 블로그

각 사례·옵션·튜닝은 다음 출처에서 검증한다.

**🇰🇷 국내** (네이버 D2, 카카오 tech, 라인 engineering, 쿠팡 engineering, 우아한기술블로그, 당근 team, 토스 tech, 야놀자 dev, 무신사 tech, 컬리 helloworld, NHN Cloud 등)

**🌍 해외** (Netflix Tech Blog, LinkedIn Engineering, Twitter Engineering, Spotify, Uber, Slack, Airbnb, Pinterest, Square 등)

**🎓 개인/컨퍼런스** (Cliff Click, Aleksey Shipilev, Gil Tene, Per Liden / JavaOne, Devoxx, QCon, KubeCon, NAVER ENGINEERING DAY, if(kakao))

전체 URL과 핵심 콘텐츠 목록은 [10-ops-scenarios/00-real-world-cases.md](./10-ops-scenarios/00-real-world-cases.md) 상단 참조.
...
| # | 폴더 | 핵심 질문 | 상태 |
|---|---|---|---|
| 00 | [00-overview](./00-overview/) | "JVM이 뭐고, 왜 만들었고, 어떻게 자랐나" — 본문 4편 + `02-deep-dive/` 부록 5편(A~E) | ✅ 완료 |
...
- 부록(`02-deep-dive/`)으로 Interpreter 구현체 비교(A), JVM 구현체 비교(B), AST(C), Opcode dispatch + Template Interpreter(D), AOT·JIT 최적화(E)
...
- [x] 00-overview 본문 4편 (`01-what-is-jvm-jre-jdk`, `02-class-compilation-flow`, `03-jvm-architecture-bigpicture`, `04-jvm-history`) + Excalidraw 13개
  - [x] `02-deep-dive/` 부록 5편 (A: Interpreter 구현체 비교 / B: JVM 구현체 비교 / C: AST / D: Opcode dispatch + Template Interpreter / E: AOT·JIT 최적화)
- [x] 01-class-lifecycle README + 본문 4편 (`01-classfile-format` ← 시니어 운영자 관점 재작성, `02-classloader-hierarchy`, `03-linking`, `04-initialization-and-unload`) + Excalidraw 4개
  - [x] 각 챕터 도입부에 "JVM 라이프사이클 안에서 이 챕터의 위치" 컨텍스트 + 중간 reminder
- [x] 02-runtime-data-areas — 본문 6편 완료
  - [x] README + 01-heap-and-tlab + 02-metaspace + 03-stack-pc-native + 04-code-cache + 05-direct-memory + 06-gc-bookkeeping
  - [ ] _excalidraw/ (각 sub-chapter 다이어그램) — 본문 ASCII로 충분히 풍부, SVG는 추후
...
- [ ] 10-ops-scenarios — 운영 시나리오 매핑집
  - [x] 00-real-world-cases.md — 빅테크 8대 장애 패턴 (네카라쿠배당토 + Netflix/LinkedIn/Twitter)
  - [ ] 각 패턴 풀버전 sub-chapter들 (01~08)
